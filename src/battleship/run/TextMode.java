...
public class TextMode {
	...
	protected static final Pattern answerPattern = Pattern.compile(
			"([a-j])\\s*(\\d+)|q", Pattern.CASE_INSENSITIVE);
	protected static final Pattern answerPatternBoatPosition = Pattern.compile(
			"([a-j])*(\\d+)*([h-v])", Pattern.CASE_INSENSITIVE);
	...
	protected static void showGrid(Cell[][] grid) {
		showGrid(grid, false);
	}
	...
	protected static boolean isFinish(Cell[][] grid) {
		for (Cell[] eachRow : grid) {
			for (Cell eachCell : eachRow) {
				if (eachCell.getShip() != null) {
					if (eachCell.getShip().isSunk() == false) {
						return false;
					}
				}
			}
	...
	protected static void placeShips(Cell[][] grid) {
		Ship[] tabShip = { new Ship("Cruiser", 4), new Ship("Frigate", 3),
				new Ship("Destroyer", 2), new Ship("Destroyer", 2) };
		...
		for (Ship ship : tabShip) {
			...
			while (true) {
				try {
					...
					char orientation = position.group(3).toUpperCase()
							.charAt(0);
	...
	protected static void play(Cell[][] grid) {
		while (true) {
			...
			if (answer.group(0).equals("Q")) {
				showGrid(grid, true);
				System.out
						.println("Ok, bye! Here's the final state of affairs:");
				break;
			} else {
	...
	protected static MatchResult askNextMove() {
		while (true) {
			System.out
					.println("Drop a bomb where? [letter+number, or Q to give up]");
			String typedAnswer = new Scanner(System.in).nextLine()
					.toUpperCase().trim();
	...
	protected static MatchResult askBoatPosition() {
		while (true) {
			System.out
					.println("Where do you want to place it ? [letter+number+orientation] (orientation : H or V)");
			String typedAnswer = new Scanner(System.in).nextLine()
					.toUpperCase().trim();
			Matcher answerMatch = answerPatternBoatPosition
					.matcher(typedAnswer);
			if (answerMatch.matches()) {
				return answerMatch.toMatchResult();
			}
